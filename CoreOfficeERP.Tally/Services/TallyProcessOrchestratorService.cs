...
namespace CoreOfficeERP.Tally.Services
{
    public class TallyProcessOrchestratorService:ITallyProcessOrchestratorService
    {
        ...
        public async Task<List<TallyProcessRequest>> ExecutePurchase(TallyPurchaseResponse data,TallyConfigResponse config,int finid, string sbillnumber,DateTime date)
        {
            ...
            try
            {
                ...
                () => _tally.CreatePurchaseVoucher(data, config,sbillnumber,date), logs, data, config, finid))
                    return logs;              
            }
            catch (Exception ex)
            {
